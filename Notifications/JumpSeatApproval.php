 ...
 */
class JumpSeatApproval extends Notification
{
    use Queueable;
    ...
    public function toDiscordChannel($notifiable): ?DiscordMessage
    {
        ...
        return $dm->webhook(setting('notifications.discord_private_webhook_url'))
            ->success()
            ->title('Jumpseat Approved: '.$this->jsr->user->ident.' - '.$this->jsr->user->name_private)
            ->author([
                'name'     => $this->jsr->approver->ident.' - '.$this->jsr->approver->name_private,
                'url'      => '',
                'icon_url' => $this->jsr->approver->resolveAvatarUrl(),
            ])
            ->fields([
